/**
 * Highlight.io React Native integration via OpenTelemetry
 * @see https://highlight.io/docs/getting-started/browser/react-native
 */
...
class ReactNativeOTLPTraceExporter implements SpanExporter {
  ...
  _convertEvent(timedEvent: TimedEvent) {
    return {
      attributes: timedEvent.attributes ? this._convertAttributes(timedEvent.attributes) : [],
  ...
  _toAnyValue(value: any): any {
    ...
    if (Array.isArray(value)) return { arrayValue: { values: value.map(this._toAnyValue) } };
    if (t === 'object' && value != null)
      return {
        kvlistValue: {
          values: Object.entries(value as object).map(([k, v]) => this._convertKeyValue(k, v)),
        },
      };
    return {};
  }
...
const resource = new Resource({
  ...
  'service.name': 'chess-academy',
});

const tracerProvider = new BasicTracerProvider({ resource });
const otlpExporter = new ReactNativeOTLPTraceExporter({ url: HIGHLIGHT_OTLP_URL });
tracerProvider.addSpanProcessor(new BatchSpanProcessor(otlpExporter));
tracerProvider.register();

export const tracer = tracerProvider.getTracer('chess-academy-tracer');
...
export function log(level: string, message: string, attributes = {}) {
...
export function error(message: string, attributes = {}) {
  const span = tracer.startSpan('highlight.log');
